...
contract('Darshan_19IT143', function(accounts) {
  var tokenInstance;

  it('initializes the contract with the correct values', function() {
    return Darshan_19IT143.deployed().then(function(instance) {
      tokenInstance = instance;
      return tokenInstance.name();
    }).then(function(name) {
      assert.equal(name, 'Darshan_19IT143', 'has the correct name');
      return tokenInstance.symbol();
    }).then(function(symbol) {
      assert.equal(symbol, '19IT143', 'has the correct symbol');
      return tokenInstance.standard();
    }).then(function(standard) {
      assert.equal(standard, 'Darshan_19IT143 v1.0', 'has the correct standard');
    });
  })
...
it("transfers token ownership", function() {
    return Darshan_19IT143.deployed()
